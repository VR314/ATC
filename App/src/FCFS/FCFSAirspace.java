        ...
        for (int i = 0; i < airport.planes.size(); i++) {
            for (int j = 0; j < airport.planes.size(); j++) {
                if (i != j) {
                    ...
                    if (Math.pow(p.coords[0] - p2.coords[0], 2) + Math.pow(p.coords[1] - p2.coords[1], 2) < 5) {
                        crashes++;
                        System.out.println("CRASH: " + p.id + " + " + p2.id);
                    } else if (Math.pow(p.coords[0] - p2.coords[0], 2) + Math.pow(p.coords[1] - p2.coords[1], 2) < 12) {
                        nearMisses++;
